...
def train(model, train_loader, criterion, optimizer, writer=None, epoch=None):
    running_loss = 0
    # model.train()
...
def test(model, test_data, writer=None, epoch=None):
    accs = []
...
def run(model, train_loader, criterion, test_data, optimizer, epochs=5):
    ...
    for epoch in range(1, epochs):
        ...
        loss = train(model=model, train_loader=train_loader, criterion=criterion, optimizer=optimizer, writer=writer,
                     epoch=epoch)

        now = datetime.datetime.now()
        if test_data is not None:
            test_acc = test(model=model, test_data=test_data, writer=writer, epoch=epoch)
